...
export async function POST(request: NextRequest) {
  try {
    ...
    forwardData.append("image", file, file.name || "camera.jpg");

    const response = await fetch(`${BACKEND_URL}/api/camera-ocr/capture`, {
      method: "POST",
      body: forwardData,
      cache: "no-store",
    });
    ...
    const message =
      error instanceof Error ? error.message : "Camera OCR request failed";
    return NextResponse.json(
      {
        success: false,
        error: message,
